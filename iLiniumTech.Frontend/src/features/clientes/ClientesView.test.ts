...
describe('ClientesView', () => {
  it('renders static MVP content with disabled actions and no runtime markers', () => {
    const wrapper = mount(ClientesView, {
      global: {
        stubs: {
    ...
    expect(wrapper.text()).toContain('PII Alta')
    expect(wrapper.text()).toContain('listado read-only minimizado')
    expect(wrapper.text()).toContain('vw_Cliente')
    expect(wrapper.text()).toContain('clientes.read')
