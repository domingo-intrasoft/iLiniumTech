...
public sealed class SqlClientesWriteRepository(
    IPolizasConnectionStringProvider connectionStringProvider,
    IPolizasExecutionContextAccessor executionContextAccessor,
    SqlClientesCommandBuilder? commandBuilder = null)
    : IClientesWriteRepository
{
    private readonly SqlClientesCommandBuilder _commandBuilder = commandBuilder ?? new SqlClientesCommandBuilder();

    public async Task<ClienteCreateResult> CreateAsync(ClienteCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("Clientes SQL create did not return a numeric identifier.");
            }
            ...
            return new ClienteCreateResult(id);
        }
        catch
        {
    ...
    public async Task<bool> UpdateAsync(string id, ClienteUpdateRequest request, CancellationToken cancellationToken)
    {
        var parsedId = ClientesWriteValidator.ValidateAndParseId(id);
        var executionContext = RequireExecutionContext();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (visibleRows <= 0)
        {
            throw new ClientesValidationException("Created MVP cliente is not visible for the active broker context.");
        }
    }
