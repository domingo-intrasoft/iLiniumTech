...
public sealed class RecibosApiTests
{
    [Fact]
    public async Task Recibos_requires_authentication()
    {
        await using var factory = new TestApiFactory();
        using var client = factory.CreateClient();

        var response = await client.GetAsync("/api/recibos");

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task Legacy_api_key_does_not_grant_recibos_read()
    {
        await using var factory = new TestApiFactory();
        using var client = factory.CreateClient();
        ...
        body.Should().NotContain("REC-2026");
        body.Should().NotContain("ConnectionStrings");
    }

    [Fact]
    public async Task Recibos_catalogs_require_exact_permission()
    {
        await using var factory = new TestApiFactory(new Dictionary<string, string?>
        {
            ["Auth:Demo:Permissions:0"] = RecibosPermissions.Read
        });
        using var client = factory.CreateClient();
        await LoginDemoAsync(client);

        var response = await client.GetAsync("/api/recibos/catalogs");
        var body = await response.Content.ReadAsStringAsync();
    ...
    [Fact]
    public async Task Recibos_search_returns_minimized_fixture_contract_without_financial_or_bank_fields()
    {
        await using var factory = new TestApiFactory(new Dictionary<string, string?>
        {
            ["Auth:Demo:Permissions:0"] = RecibosPermissions.Catalogs,
            ["Auth:Demo:Permissions:1"] = RecibosPermissions.Read
        });
        using var client = factory.CreateClient();
        await LoginDemoAsync(client);

        var catalogs = await client.GetAsync("/api/recibos/catalogs");
        var catalogsBody = await catalogs.Content.ReadAsStringAsync();
        var response = await client.GetAsync("/api/recibos?page=1&pageSize=2&situacion=Cobrado");
        var body = await response.Content.ReadAsStringAsync();
        ...
        body.Should().Contain("REC-2026-0002");
        body.Should().Contain("POL-2026-0002");
        body.Should().NotContain("REC-2026-0001");
    ...
    [Fact]
    public async Task Recibos_rejects_sort_fields_outside_whitelist()
    {
        await using var factory = new TestApiFactory(new Dictionary<string, string?>
        {
            ["Auth:Demo:Permissions:0"] = RecibosPermissions.Read
        ...
        using var client = factory.CreateClient();
        await LoginDemoAsync(client);
        client.DefaultRequestHeaders.Add("X-Correlation-Id", "recibos-sort-validation");

        var response = await client.GetAsync("/api/recibos?sort=SELECT%20*%20FROM%20SecretTable:desc");
        var body = await response.Content.ReadAsStringAsync();

        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        response.Headers.GetValues("X-Correlation-Id").Should().Contain("recibos-sort-validation");
        body.Should().Contain("RECIBOS_VALIDATION_ERROR");
        body.Should().Contain("\"correlationId\":\"recibos-sort-validation\"");
