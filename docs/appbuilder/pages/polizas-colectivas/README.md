# Polizas / Colectivas - scope estatico MVP
...
Este documento corrige el inventario de paginas para reflejar que `Colectivas` ya existe como scope estatico del frontend actual.
...
- `src/layout/appNavigation.ts` contiene la entrada hija `Colectivas` bajo `Polizas`.
- La ruta declarada es `/polizas/colectivas`.
- Existe pantalla Vue estatica en `src/features/polizas-scopes/PolizasColectivasView.vue`.
...
- Por el nombre funcional, `Colectivas` podria ser una vista, filtro o scope de polizas asociado a polizas colectivas.
...
- SDD funcional de `Polizas / Colectivas`.
- Regla funcional validada para identificar polizas colectivas.
- Permiso iLiniumTech aprobado, por ejemplo `polizas.scope.colectivas.read` si se confirma el modelo de scope.
...
`Colectivas` queda inventariada como scope estatico bajo Polizas. Puede permanecer visible para orientar pruebas de navegacion, pero debe mostrarse y tratarse como bloqueada para datos reales hasta cumplir los bloqueos anteriores.
...
- Frontend estatico protegido en `/polizas/colectivas`, con `PolizasColectivasView.vue`.
- Test compartido: `PolizasScopesView.test.ts`.
- Estado de navegacion esperado: `blockedSdd`.
- No existe SDD funcional de Colectivas ni permiso backend efectivo.

Componentes/patrones detectados:

- Usa `MvpPageShell` y una definicion estatica `MvpPageDefinition`.
- Muestra acciones deshabilitadas: filtrar colectivas, ver poliza y exportar.
- Declara explicitamente que no infiere filtros por ramo, division, tipo o etiqueta.
- El test verifica ausencia de marcadores runtime peligrosos y menciona permiso candidato `polizas.scope.colectivas.read`.

Bloqueos de PII, finanzas y workflows:

- No conectar a `/api/polizas` mediante texto libre de scope.
- No exponer cliente, documento, riesgo, asegurados, colectivos, importes ni exportaciones.
- No activar detalle real ni acciones sobre polizas colectivas sin SDD propia.

Permisos candidatos:

- `polizas.scope.colectivas.read` si producto confirma que Colectivas es scope de Polizas.
- Alternativa futura: permisos propios de un vertical si Colectivas deja de ser simple scope.

Dependencias UAT/DBA:

- Confirmar regla funcional para identificar polizas colectivas.
- Confirmar si sera filtro de Polizas, pagina hija o vertical independiente.
- Confirmar origen SQL/vista y whitelists.
- Confirmar minimizacion de PII e impacto multi-tenant.

Tareas futuras pequenas:

- `producto-polizas-colectivas-scope-decision`: decidir modelo funcional.
- `producto-sdd-polizas-colectivas-readonly`: redactar SDD si se prioriza.
- `backend-polizas-colectivas-filter-design`: proponer enum/filtro cerrado, no implementarlo sin SDD.
- `frontend-polizas-colectivas-static-polish`: solo mejorar copy/test del placeholder.
