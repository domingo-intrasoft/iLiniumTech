...
Nombre: Verificar/adaptar frontend Suplementos a API con fallback fixture.

Objetivo:

- Confirmar que `/suplementos` consume API cuando `VITE_USE_BACKEND=true`.
...
- Mantener detalle, exportacion, escritura, workflows, banco, adjuntos, documentos, importes y datos sensibles bloqueados.
...
- `docs/qa/suplementos-mvp-evidence.md`
- `docs/qa/suplementos-sql-readonly-local-evidence.md`
...
- detalle, exportacion, escrituras, workflow, adjuntos, documentos, banco, importes, textos libres o PII real;
...
3. Revisar si el composable/servicio de `Suplementos` usa API con `VITE_USE_BACKEND=true`.
...
- `/suplementos` usa API en modo backend y fixture solo en modo offline/test explicito.
- No se introducen datos sensibles, detalle/exportacion/escrituras, workflows, adjuntos, banco, importes ni SQL.
- Tests frontend dirigidos y build quedan OK.
- Evidencia QA explica modo API/fallback y riesgos residuales.

Riesgo de conflicto: medio; tocar solo `Suplementos` frontend y servicios necesarios.
...
| `T-309-SUPLEMENTOS-FE-API-ADAPTER-VERIFY` | `READY_ACTIVE` | Verificar/adaptar frontend Suplementos con fallback fixture solo cuando `VITE_USE_BACKEND=false`. | `src/features/suplementos/**` | `T-124`/`T-306` cerradas |
