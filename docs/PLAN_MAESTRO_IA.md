...
- `Clientes` tiene SDD draft abierta como tercera pagina posterior a Polizas: `docs/sdd/specs/iLiniumTech/SDD-2026-010-clientes-read-only.md`. El alcance propuesto es solo listado read-only minimizado; ficha, tabs relacionadas, documento, contacto, direccion, banco, metricas, exportacion y escrituras quedan fuera hasta SDD posterior.
- `Agenda` tiene SDD draft abierta como cuarta pagina posterior a Polizas: `docs/sdd/specs/iLiniumTech/SDD-2026-011-agenda-read-only.md`. El alcance propuesto es solo listado read-only por rango; calendario interactivo, detalle, descripcion larga, participantes, drag/drop, reprogramacion, exportacion y escrituras quedan fuera hasta SDD posterior.
- `Propuestas` tiene SDD draft abierta como quinta pagina posterior a Polizas: `docs/sdd/specs/iLiniumTech/SDD-2026-012-propuestas-read-only.md`. El alcance propuesto es solo listado read-only minimizado; detalle, cliente real, importes, documentos, tarificacion, emision, conversion, exportacion, workflows e integraciones quedan fuera hasta SDD posterior.
- `Suplementos` tiene SDD draft abierta como sexta pagina posterior a Polizas: `docs/sdd/specs/iLiniumTech/SDD-2026-013-suplementos-read-only.md`. El alcance propuesto es solo listado read-only minimizado; detalle, tabs por tipo, PII, importes, banco, documentos, recibos/declaraciones relacionados, exportacion, workflows y escrituras quedan fuera hasta SDD posterior.
...
5. Priorizar las siguientes superficies por valor demostrable y bajo riesgo: `Siniestros` listado read-only minimizado segun `SDD-2026-008`, `Recibos` listado read-only minimizado segun `SDD-2026-009`, `Clientes` listado read-only minimizado segun `SDD-2026-010`, `Agenda` listado read-only por rango segun `SDD-2026-011`, `Propuestas` listado read-only minimizado segun `SDD-2026-012`, `Suplementos` listado read-only minimizado segun `SDD-2026-013`, despues liquidaciones/reporting. El detalle ampliado de Polizas queda como mejora posterior con SDD si toca datos sensibles o UX/contrato.
