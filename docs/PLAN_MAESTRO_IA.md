...
5. Priorizar las siguientes superficies por valor demostrable y bajo riesgo: `Siniestros` listado read-only minimizado segun `SDD-2026-008`, `Recibos` listado read-only minimizado segun `SDD-2026-009`, `Clientes` listado read-only minimizado segun `SDD-2026-010`, `Agenda` listado read-only por rango segun `SDD-2026-011`, `Propuestas` listado read-only minimizado segun `SDD-2026-012`, despues Suplementos. El detalle ampliado de Polizas queda como mejora posterior con SDD si toca datos sensibles o UX/contrato.
